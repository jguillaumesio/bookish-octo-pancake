...
exports.normaliseString = string => {
    ...
    return string.replace(/[^A-Za-z0-9\[\] ]/g, a => latin_map[a]||a);
}

exports.logError = (file, error) => {
    const fileName = `${appRoot}/public/log.txt`;
    if (fs.existsSync(fileName)) {
        fs.appendFileSync(fileName, `${file}: ${error}`);
    } else {
        fs.writeFileSync(fileName, `${file}: ${error}`);
    }
}